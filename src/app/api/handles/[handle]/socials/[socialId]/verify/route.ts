...
import { logApiUsage, verifyTelegramSocialLink } from "@/lib/adminStore";
...
export async function POST(req: NextRequest, { params }: Context) {
  ...
  if (!payload) {
    ...
    await logApiUsage({
      endpoint: "/api/handles/[handle]/socials/[socialId]/verify",
      method: "POST",
      status: 401,
  ...
  try {
    const social = await verifyTelegramSocialLink({
      userId: payload.uid,
      handle,
      socialId,
    });
